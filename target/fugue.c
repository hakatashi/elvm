...
static void init_state_fugue(Data* data) {
...
static void fugue_emit_func_prologue(int func_id) {
  emit_line("");
...
static void fugue_emit_func_epilogue(void) {
  dec_indent();
  emit_line("pc += 1");
  dec_indent();
  dec_indent();
...
static void fugue_emit_pc_change(int pc) {
  emit_line("");
  dec_indent();
...
static void fugue_emit_inst(Inst* inst) {
  switch (inst->op) {
  case MOV:
...
void target_fugue(Module* module) {
  init_state_fugue(module->data);

  int num_funcs = emit_chunked_main_loop(module->text,
                                         fugue_emit_func_prologue,
                                         fugue_emit_func_epilogue,
                                         fugue_emit_pc_change,
                                         fugue_emit_inst);
